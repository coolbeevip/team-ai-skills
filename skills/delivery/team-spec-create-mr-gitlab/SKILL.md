...
先读取 `team-spec/config.yml`，应用语言、访问策略和版本管理配置。文件不存在或正式创建 MR 所需字段无法从参数与 Git 证据唯一确定时，先使用 `team-config-init` 创建或增量补全；本技能不得自行回写配置。MR 语言按“本次 `--language` 或用户明确指定 > `version_control.language` > 顶层 `language` > `en-US`”确定。source/target project、remote 和主干优先级为：用户参数 > 已确认配置 > git 证据 > 询问用户。
...
- 标题来自显式参数或 PRD 一级标题。
- 正文从完整 Spec/PRD 生成。
- PRD/Task 语言与 MR 语言不同时，生成对应语言的标题和完整正文，并通过 `--title`、`--body-file` 传入；不修改源文档。Task ID、commit SHA、代码标识符、命令、路径和专有名词保持原样。
...
MR 合并后，提醒用户同步本地主干，确保下次创建 Spec 分支时基于最新代码：
