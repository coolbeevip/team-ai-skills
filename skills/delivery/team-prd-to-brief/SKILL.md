...
description: 将结构化 PRD 转换为适合需求、研发和项目管理评审的简报。Turn structured PRDs into review briefs for product, engineering, and project management.
license: MIT
metadata:
  author: coolbeevip
  version: "1.0"
...
这个技能用于把 `team-spec-to-prd` 生成的 AI 结构化 PRD，转译为适合需求、研发和项目管理人员阅读、评审和达成共识的评审简报。它解决的问题是：PRD 可以作为 `team-prd-to-tasks` 的机器输入，但不一定适合作为人类评审会的沟通材料。

默认输出是 Markdown Brief，但内容组织应接近演示文稿或评审会材料：结论先行、信息分层、每一节都能支撑一次讨论。它不是 PRD 的替代品，也不是工程拆解技能。

## 触发边界

- 适合触发：PRD 已存在，但需要转换为人类评审简报、会议对齐或需求研发沟通材料。
- 不适合触发：用户要固化 PRD 时，转交 `team-spec-to-prd`；用户要拆工程 Task 时，转交 `team-prd-to-tasks`。

## 运行时配置

开始前读取目标项目根目录的 `team-spec/config.yml`。文件不存在或缺少本技能写入所需字段时，先使用 `team-config-init` 创建或增量补全；本技能不得自行创建或回写配置。只读预览可按用户本轮语言继续，但写入评审简报前必须完成所需配置。

语言优先级：用户本轮明确指定 > 配置中的 `language`。

执行要求：

- 对话回复与评审简报 `team-spec/active/{slug}/prd/brief.md` 均使用 `language`。
...
- `team-spec/active/{slug}/prd/brief.md`：面向人类对齐的演示文稿式评审简报。
- 对话中的材料摘要：目标、关键范围、待决问题和建议评审关注点。

评审简报不修改 PRD 内容。PRD 仍是工程拆解的权威输入；评审简报用于帮助人类快速理解、讨论和确认 PRD。
...
- **不替代 PRD**：不要改变 PRD 的权威内容；发现 PRD 缺陷时，在评审简报中指出，并建议回到对应上游技能修正。
- **适合会议使用**：每一节应能在评审会上直接展开讨论，避免长篇背景堆叠。

## 评审简报表达

公共语言规则由 `writing_style.guide` 统一提供。本技能只补充评审简报特有的要求：

- 像熟悉需求的同事为评审会准备材料，不把 PRD 换一种格式机械复述。
- 只抽取影响范围确认、关键决策、工程判断和风险讨论的内容。
- 每一节落到可讨论的结论、分歧、负责人或下一步，不用态度性总结代替决策。
- 能指向 PRD、评审报告、决策记录或用户原话时直接写来源；证据不足时列为待确认。
...
7. 按演示文稿式结构生成评审简报，优先使用短段落、列表和明确标题。
8. 按 Blocker / Risk / Discussion 分级整理冲突和待决问题。
9. 按公共写作风格和“评审简报表达”复核并改写材料，删除空泛判断、模板句式、聊天痕迹和无证据推断。
10. 将材料写入 `team-spec/active/{slug}/prd/brief.md`。
...
- `team-spec/active/{slug}/prd/brief.md` 已生成。
...
- 明确说明 PRD 仍是 `team-prd-to-tasks` 的权威输入，评审简报只服务于人类讨论。
...
- PRD 是否需要返回上游修订。
- 有序号的下一步选项：进入对齐评审、返回上游修订，或基于权威 PRD 执行 `team-prd-to-tasks`。
