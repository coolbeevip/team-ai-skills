...
这个技能用于在 `team-issue-implement` 和 `team-issue-verify` 后，把当前 issue 分支推送到 GitHub，并创建关联 issue 的 Pull Request。它关注“少出错、可预览、可追踪”，避免手动复制 issue 编号、标题和正文时遗漏关联。
...
- `--execute` 时先 push 当前分支，再创建 GitHub Pull Request。
...
4. 用户确认后，用固定脚本追加 `--execute` 推送分支并创建 PR。
5. 输出 PR URL、关联 issue、source/target branch、验证结果和下一步建议。
