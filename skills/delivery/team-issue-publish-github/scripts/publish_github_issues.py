...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--remote",
        help="Force a git remote name for repo inference. Defaults to team-spec/config.yml version_control.target_remote when present.",
    )
    parser.add_argument(
        "--force",
        action="store_true",
...
def infer_repo(args: argparse.Namespace) -> str:
    ...
    if configured_remote:
        configured = as_repo(configured_remote)
        if not configured:
            raise SystemExit(
                "team-spec/config.yml version_control.target_remote is not a valid "
                f"GitHub remote: {configured_remote}"
            )
        return configured[1]
