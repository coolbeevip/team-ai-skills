...
这个技能用于把 `team-prd-to-issues` 生成的本地 issue 草稿，批量发布到 GitLab Issues。它关注“可重复执行、可追踪、可恢复”，避免重复创建和依赖顺序错误。
...
- 仓库定位：`namespace/project` 或可唯一定位项目的项目 ID。
...
1. 确认 slug、项目、平台地址、token 来源与权限范围。
