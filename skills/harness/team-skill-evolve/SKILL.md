...
- 不适合触发：用户要维护某个业务项目中的 Codex 运行时检索层时，转交 `team-archive-distill`；用户只是执行普通产品、交付或技术债流程时，使用对应业务技能。
...
不要把一次性反馈、真实业务需求、PRD、风险报告或工程 Task 写入本仓库。需要沉淀反馈时，优先建议使用外部 Issue、Notion 或用户指定的业务项目工作区。
