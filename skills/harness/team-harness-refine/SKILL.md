...
11. 对更新后的 harness 做一次轻量复核：入口是否更短、更清晰，目录和文件名是否稳定，新增文档是否能被入口找到，命令和验证说明是否仍然可执行。
...
- 如果文档说法与代码事实冲突，应优先修文档；如果代码或工具本身阻碍 agent 工作，再记录到 `harness-debt.md`。
...
- 关键 harness 内容已经根据当前代码、脚本、配置、测试或 CI 事实做过校验；未能确认的内容已标注或记录为 harness debt。
