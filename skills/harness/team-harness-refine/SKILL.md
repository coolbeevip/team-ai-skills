...
- `docs/agent-harness/harness-review.md`：当本轮主要是审查、诊断或给改进建议时，必须写入本轮发现、证据、建议落点和后续动作。
...
- Harness 文档层：`AGENTS.md`、`CLAUDE.md`、`docs/agent-harness/`，用于立即改善 agent 如何理解项目、运行命令和处理失败。
- Spec 流程层：`team-spec/active/spec/refine/{slug}.md`，用于承接需要工程实现、工具修复、测试补齐、CI 治理、架构文档重构或环境治理的 harness debt。

不要把可拆 issue 的工程工作只留在 `docs/agent-harness/harness-debt.md`。`harness-debt.md` 可以作为索引和运行时记录，但需要进入交付流程的事项必须落到 `team-spec/active/spec/refine/{slug}.md`，并推荐下一步使用 `team-tech-debt-review`。

落盘规则：

- 新建 harness：至少创建或更新 `AGENTS.md`，并创建 `docs/agent-harness/index.md`、`commands.md`、`verification.md` 中与本轮证据相关的文件。
- 审查现有 harness：至少更新 `docs/agent-harness/harness-review.md`，并把可直接固化的建议写入对应 harness 文档。
- 根据失败案例更新 harness：至少更新 `known-failures.md`；如果问题仍未解决，还要更新 `harness-debt.md`。
- 根据开发任务更新 harness：至少更新与任务相关的 `commands.md`、`verification.md`、`architecture-map.md`、`coding-rules.md` 或 `review-rubric.md` 之一。
- 发现无法通过文档解决的问题时，不能只在最终回复中建议；必须写入 `harness-debt.md`。
...
8. 更新 `AGENTS.md` / `CLAUDE.md` 和相关 `docs/agent-harness/` 文档；已有内容应增量修订，不要无理由重写。
9. 如果本轮主要产出是建议或诊断，写入 `docs/agent-harness/harness-review.md`，并把可执行建议分流到具体文档或 `harness-debt.md`。
10. 对更新后的 harness 做一次轻量复核：入口是否更短、更清晰，命令是否仍然可执行，新增文档是否能被入口找到。
11. 将无法通过文档解决的问题写入 `docs/agent-harness/harness-debt.md`。
