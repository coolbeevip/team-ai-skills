...
- `AGENTS.md` 能准确路由到现有的核心检索文件。
- 新增或更新内容只属于入口约束、失败记忆、验证 harness 或任务入口。
- 每条非平凡记录都有证据来源，未验证内容已明确标注。
- 没有把普通项目文档、完整架构地图或一次性任务噪音写入 harness。
- 已用一个真实任务或失败案例检查检索层是否能减少错误或无效探索。
...
- 目标项目和 harness 目录。
- 创建或更新的 `AGENTS.md` 与核心检索文件。
- 本轮补充的入口约束、失败记忆、验证策略或任务入口摘要。
- 使用的证据、待验证内容和未解决风险。
- 实际执行的轻量自检或真实任务验证结果。
- 没有生成的非核心文档及原因。
