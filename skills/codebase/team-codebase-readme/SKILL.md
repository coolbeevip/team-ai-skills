...
- 不适合触发：用户要生成完整架构文档、功能设计、API 参考、教程或业务汇报材料时，应另行确定对应工作流；当前技能库不提供通用功能设计文档生成技能。
- 本技能与 `team-codebase-onboarding` 是并列能力，不读取其产物，也不要求创建 `team-spec` slug。

## 运行时配置

开始前读取目标项目根目录的 `team-spec/config.yml`。文件不存在时可以继续纯审阅和只读事实盘点；准备创建或改写 README 且配置缺失或缺少本轮必需字段时，先使用 `team-config-init` 创建或增量补全，本技能不得自行创建或回写配置。

语言优先级为：用户本轮明确指定 > 配置中的 `language` > 目标 README 的既有主要语言。如果配置存在 `access_policy`，读取源码、配置、文档或写入 README 前先应用目录访问边界。
