...
开始前读取目标项目根目录的 `team-spec/config.yml`。文件不存在时可以继续纯只读扫描；准备写入 onboarding 产物且配置缺失或缺少本轮必需字段时，先使用 `team-config-init` 创建或增量补全，本技能不得自行创建或回写配置。

- 文档与用户交互语言按“用户本轮明确指定 > 配置中的 `language` > 用户当前使用语言”确定。
- 如果配置存在 `access_policy`，读取源码、文档或写入产物前先应用其目录访问边界；无法确认读取或写入权限时停止对应操作，不扩大扫描范围。
- 用户明确要求目标仓库只读或指定外部输出目录时，保持目标仓库只读，并在分析计划和最终回复中记录实际写入范围。
...
8. 所有生成文档使用运行时确定的语言，文风为说明性、报告式、面向接手项目的开发者；模板中的固定中文标题按需翻译，但字段含义和证据合同保持不变。
...
- onboarding 输出根目录、slug 和实际生成的主要文档。
- 扫描范围、排除范围、证据来源和总体置信度。
- 主要功能、架构、接口、数据、配置、测试与风险结论。
- `[TODO]`、`[ASK USER]` 和声明意图与源码现实之间的偏差。
- 是否执行了显式 `docs/` 导出，以及允许写入范围之外的文件未被修改。
- 建议的下一步走读入口。
