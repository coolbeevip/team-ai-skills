...
- 设计意图与应用场景：设计意图、服务对象、触发场景、问题边界、成功信号和异常场景。
- 当前走读层级：L0 设计意图与应用场景、L1 概览、L2 入口调用链、L3 数据/配置/接口、L4 测试验证或 L5 修改影响面。
- 专有名词解释：当文档出现项目、业务、领域、协议、平台或团队约定中的专有名词时必须包含；每项说明含义、当前问题中的作用和证据来源，无法确认时标注“推断”或 `[TODO]`。若本轮没有专有名词，可省略。
- 本次结论。
- 建议阅读路径。
- 核心解释或实现链路。
- 来源文件。
