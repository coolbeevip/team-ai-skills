...
开始前读取目标项目根目录的 `team-spec/config.yml`。文件不存在时可以继续即时问答和纯只读走读；准备写入 `codebase-walk/` 产物且配置缺失或缺少本轮必需字段时，先使用 `team-config-init` 创建或增量补全，本技能不得自行创建或回写配置。

- 文档与用户交互语言按“用户本轮明确指定 > 配置中的 `language` > 用户当前使用语言”确定。
- 如果配置存在 `access_policy`，读取 onboarding 产物、源码、配置、测试或写入走读记录前先应用其目录访问边界。
- 权限不足时仍可基于允许读取的已有证据回答，但必须说明范围限制；不得越过访问策略补扫源码或写入受限目录。
...
11. 需要沉淀时只写 `team-spec/active/{slug}/design/codebase-walk/`，并遵守运行时访问策略；不得修改业务源码、测试、配置或 onboarding 产物。
